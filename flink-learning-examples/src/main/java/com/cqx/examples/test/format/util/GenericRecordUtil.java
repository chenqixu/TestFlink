...
import com.cqx.examples.test.format.bean.AvroLevelData;
import com.cqx.examples.test.format.bean.AvroRecord;
import com.cqx.examples.test.format.bean.DefaultBean;
...
public class GenericRecordUtil implements Serializable {
    ...
    public GenericRecordUtil(String schemaUrl, String cluster_name, String group_id) {
        // 初始化schema工具类
        schemaUtil = new SchemaUtil(schemaUrl, cluster_name, group_id);
    }
    ...
    /**
     * 把字节数反序列化成GenericRecord
     *
     * @param topic
     * @param msgByte
     * @return
     */
    public GenericRecord binaryToRecord(String topic, byte[] msgByte) {
        RecordConvertor recordConvertor = recordConvertorMap.get(topic);
        if (recordConvertor != null) {
            return recordConvertor.binaryToRecord(msgByte);
        } else {
            throw new RuntimeException(String.format("话题[%s]未注册, schema为空!", topic));
        }
    }
